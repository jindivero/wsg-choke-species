...
###LOAD DATA ###
prepare_data <- function(spc,sci_name,mi,iphc, file_name){
...
#  }
  ...
  ###Add in situ data####
    ...
    Eo1 <-  0.33 #MI_pars[1,3] #Median
    Eo2 <-  0.1  #Eo1-(MI_pars[2,3]*1.28)    #low 90th percentile, close to zero
    Eo3 <-  .75  #Eo1+(MI_pars[2,3]*1.28)    #High 90th percentile, close to zero
   ...
   # dat$mean_weight <- mean(dat$median_weight, na.rm=T)
    ...
    #Reorder columns
    if(iphc==F){
      dat <- relocate(dat, scientific_name, common_name, survey, year, date, doy, month, depth, longitude, latitude, cpue_kg_km2, cpue_kg_km2_sub, salinity_psu, temperature_C, sigma0_kgm3,do_mlpL, O2_umolkg, invtemp, po2, mi1,mi2,log_depth_scaled, log_depth_scaled2, X, Y, p1,p2,p3,p4,median_weight, haul_weight, nlength, pass, vessel, tow, bottom_temperature_c)
    }
    if(iphc==T){
      dat <- relocate(dat, scientific_name, common_name, survey, year, date, doy, month, depth, longitude, latitude, cpue_kg_km2, cpue_kg_km2_sub,cpue_weight, cpue_count, salinity_psu, temperature_C, sigma0_kgm3, do_mlpL, O2_umolkg, invtemp, po2,mi1,mi2,log_depth_scaled, log_depth_scaled2, X, Y, p1,p2,p3,p4,median_weight, haul_weight, nlength, pass, vessel, tow, bottom_temperature_c)
