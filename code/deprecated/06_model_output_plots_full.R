...
for(i in 1:nrow(bp_est4)){
  ...
  #Scale MI calculated in points to the fitting dataset
...
dat2plot2 <- filter(dat2plot2, (latitude>(min_lat-0.5))&(latitude<(max_lat+0.5)))
dat2plot2 <- filter(dat2plot2, year==2021)

dat2plot <- bind_rows(dat2plot, dat2plot2)

ylims <- c(min(cc_grid$Y)*1000, max(bc_grid$Y)*1000)
xlims <- c(min(bc_grid$X)*1000, max(cc_grid$X)*1000)
...
dat2plot_wa <- filter(dat2plot, latitude>46&latitude<48.5)
ggplot(us_coast_proj) + geom_sf() +
  geom_point(dat2plot_wa,mapping=aes(x=X*1000, y=Y*1000, colour=est_effect_prop), size=0.5)+
  #geom_point(filter(dat2plot, est_effect_raw==max_effect),mapping=aes(x=X*1000, y=Y*1000), colour="#440154FF", size=0.5)+
  #geom_point(filter(dat2plot, est_effect_raw<max_effect),mapping=aes(x=X*1000, y=Y*1000, colour=est_effect_prop), size=0.5)+
  xlim(min(dat2plot_wa$X)*1000, max(dat2plot_wa$X)*1000)+
...
dat2plot <- filter(grid_biomass, depth_m<(depth_limit+200))
dat2plot <- filter(dat2plot, (latitude>(min_lat-0.5))&(latitude<(max_lat+0.5)))
dat2plot <- filter(dat2plot, year==2021|year==2015)

dat2plot2 <- filter(grid_abundance, depth_m<(depth_limit+200))
dat2plot2 <- filter(dat2plot2, (latitude>(min_lat-0.5))&(latitude<(max_lat+0.5)))
dat2plot2 <- filter(dat2plot2, year==2021|year==2015)

dat2plot <- bind_rows(dat2plot, dat2plot2)
dat2plot_wa <- filter(dat2plot, latitude>46&latitude<48.5)

dat2plot_wa <- filter(dat2plot_wa, (common_name=="canary rockfish"|common_name=="pacific halibut"|common_name=="redstripe rockfish"|common_name=="shortspine thornyhead"))
...
ggplot(us_coast_proj) + geom_sf() +
  ...
  xlim(min(dat2plot_wa$X)*1000, max(dat2plot_wa$X)*1000)+
  ylim(min(dat2plot_wa$Y)*1000, max(dat2plot_wa$Y)*1000)+
  facet_wrap(common_name~year, ncol=4)+
  theme_minimal(base_size=12)+
  xlab("Longitude")+
  ylab("Latitude")+
  theme(legend.position="top")+
...
##Make quadrants
grids <- filter(cc_grid, latitude>46&latitude<48.5)
#Make st object
polygons <- st_as_sf(grids, coords = c("X", "Y"), crs = 4326)
grd = st_make_grid(polygons,n=100,crs = 4326 )
index <- which(lengths(st_intersects(grd, polygons)) > 0)

fishnet_grid_sf = st_sf(area_grid) %>%
  # add grid ID
  mutate(grid_id = 1:length(lengths(area_grid)))

#Convert back to dataframe
fishnet_grid <- as.data.frame(fishnet_grid_sf)
