  ...
  for(i in 1:length(dats)){
    ...
    if(quad_depth_m1_6){
      ...
      if(dat_names[i]!="coastwide_iphc"){
        formula =   paste0("catch ~ -1 + year+", depth_term)
        st_type="iid"
      } else {
        formula = paste0("catch ~ -1 + year+region+", depth_term)
        if(!spatio_temp){
          st_type <- "off"
          formula = paste0("catch ~ -1 year+ region+", depth_term)
        } else{
      ...
      if(dat_names[i]!="coastwide _iphc"){
        formula =  paste0("catch ~ -1+year+temp_scaled + temp_scaled2+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula =paste0("catch ~ -1 + year+region+temp_scaled + temp_scaled2+", depth_term)
        } else{
          st_type="iid"
          formula =paste0("catch ~ -1 + year+region+temp_scaled + temp_scaled2 +", depth_term)
        }
      ...
      print("fitting m3")
      if(dat_names[i]!="coastwide _iphc"){
        formula =  paste0("catch ~ -1 +year+breakpt(mi1_s)+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula = paste0("catch ~ -1 + year+region +breakpt(mi1_s)+", depth_term)
        } else{
          st_type="iid"
          formula = paste0("catch ~ -1 + year+region+breakpt(mi1_s)+", depth_term)
        }
      ...
      start = Sys.time()
      if(dat_names[i]!="coastwide _iphc"){
        formula =   paste0("catch ~ -1+year +breakpt(mi2_s)+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula =  paste0("catch ~ -1 + year+region +breakpt(mi2_s)+", depth_term)
        } else{
          st_type="iid"
          formula =  paste0("catch ~ -1 + year+region+breakpt(mi2_s)+", depth_term)
        }
      ...
      start = Sys.time()
      if(dat_names[i]!="coastwide _iphc"){
        formula =   paste0("catch ~ -1 +year+breakpt(po2_s)+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula =  paste0("catch ~ -1 + year+region +breakpt(po2_s)+", depth_term)
        } else{
          st_type="iid"
          formula = paste0("catch ~ -1 +year+region+breakpt(po2_s)+", depth_term)
        }
    ...
    if(cub_depth_m7_16){
      ...
      if(dat_names[i]!="coastwide _iphc"){
        ...
        if(!spatio_temp){
          st_type <- "off"
          formula =paste0("catch ~ -1 + year+region +year+temp_scaled + temp_scaled2 +", depth_term)
        } else{
          st_type="iid"
          formula =paste0("catch ~ -1 + year+region +temp_scaled + temp_scaled2 +", depth_term)
        }
      ...
      if(dat_names[i]!="coastwide _iphc"){
        ...
        if(!spatio_temp){
          st_type <- "off"
          formula =paste0("catch ~ -1 + year+region +year+breakpt(mi1_s)+", depth_term)
        } else{
          st_type="iid"
          formula =paste0("catch ~ -1 + year+region +breakpt(mi1_s)+", depth_term)
        }
      ...
      if(dat_names[i]!="coastwide _iphc"){
        ...
        if(!spatio_temp){
          st_type <- "off"
          formula = paste0("catch ~ -1 + year+region+year+breakpt(mi2_s)+", depth_term)
          st_type="iid"
      ...
      if(dat_names[i]!="coastwide _iphc"){
        ...
        formula = paste0("catch ~ -1 + year+region+year +breakpt(po2_s)+", depth_term)
        if(!spatio_temp){
          st_type <- "off"
        } else{
          st_type="iid"
      ...
      print("fitting m13")
      if(dat_names[i]!="coastwide _iphc"){
        formula = paste0("catch ~ -1 +year+breakpt(mi1_s)+ +temp_scaled + temp_scaled2+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula = paste0("catch ~ -1 + year+region +year+breakpt(mi1_s)+ +temp_scaled + temp_scaled2+", depth_term)
        } else{
          st_type="iid"
      ...
      start = Sys.time()
      if(dat_names[i]!="coastwide _iphc"){
        formula = paste0("catch ~ -1+year+breakpt(mi2_s)+temp_scaled + temp_scaled2+", depth_term)
        st_type="iid"
      } else {
        formula =  paste0("catch ~ -1 + year+region+year +breakpt(mi2_s)+", depth_term)
        if(!spatio_temp){
      ...
      start = Sys.time()
      if(dat_names[i]!="coastwide _iphc"){
        formula = paste0("catch ~ -1+year +breakpt(mi3_s)+ temp_scaled + temp_scaled2+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula = paste0("catch ~ -1 + year+region +year+breakpt(mi3_s)+temp_scaled + temp_scaled2+", depth_term)
        } else{
          st_type="iid"
          formula =  paste0("catch ~ -1 + year+region +breakpt(mi3_s)+temp_scaled + temp_scaled2+", depth_term)
        }
      }
      m15 <- try(sdmTMB(
        formula = as.formula(formula),
      ...
      start = Sys.time()
      if(dat_names[i]!="coastwide _iphc"){
        formula = paste0("catch ~ -1+year +breakpt(po2_s)+ temp_scaled + temp_scaled2+", depth_term)
        st_type="iid"
      } else {
        if(!spatio_temp){
          st_type <- "off"
          formula = paste0("catch ~ -1+year+region +year+breakpt(po2_s)+ temp_scaled + temp_scaled2+", depth_term)
        } else{
          st_type="iid"
          formula = paste0("catch ~ -1+year+region +breakpt(po2_s)+ temp_scaled + temp_scaled2+", depth_term)
        }
      }
      m16 <- try(sdmTMB(
        formula = as.formula(formula),
